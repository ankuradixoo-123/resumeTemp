...
function App() {
  const [formData, setFormData] = useState({
    ...
    objective: "",
    experience: [
      { organization: "", designation: "", description: "", details: [""] },
    ],
  ...
  useEffect(() => {
    setChangesReflected(resumeData && !isEqual(formData, resumeData));
  }, [formData, resumeData]);

  const handleChange = (index, e, fieldName, section = null) => {
    const { value } = e.target;
    if (section) {
      const updatedSection = cloneDeep(formData[section]);
  ...
  const addArrayField = (section) => {
    const newField =
      section === "experience"
        ? { organization: "", designation: "", description: "", details: [""] }
        : section === "projects"
        ? { title: "", synopsis: "", techUsed: "", responsibilities: "" }
  ...
  const handleDetailChange = (expIndex, detailIndex, e) => {
    const { value } = e.target;
    const updatedExperience = cloneDeep(formData.experience);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/submit-resume",
        formData
      );
    ...
    <div className="App bg-green-700">
      ...
      <form onSubmit={handleSubmit}>
          ...
          {formData.experience.map((exp, index) => (
            <div key={index} className="experience-entry">
              <input
                ...
                onChange={(e) =>
                  handleChange(index, e, "organization", "experience")
                }
              />
              <input
                type="text"
                ...
                onChange={(e) =>
                  handleChange(index, e, "designation", "experience")
                }
              />
              <textarea
                name="description"
                ...
                onChange={(e) =>
                  handleChange(index, e, "description", "experience")
                }
              />
              <button type="button" onClick={() => addDetailField(index)}>
                Add Detail
              </button>
          ...
          {formData.projects.map((project, index) => (
            <div key={index} className="project-entry">
              <input
                ...
                value={project.title}
                onChange={(e) =>
                  handleChange(index, e, "title", "projects")
                }
              />
              <textarea
                name="synopsis"
                ...
                onChange={(e) =>
                  handleChange(index, e, "synopsis", "projects")
                }
              />
              <input
                type="text"
                ...
                onChange={(e) =>
                  handleChange(index, e, "techUsed", "projects")
                }
              />
              <textarea
                ...
                onChange={(e) =>
                  handleChange(index, e, "responsibilities", "projects")
                }
              />
              <button
                type="button"
                onClick={() => removeArrayField(index, "projects")}
              >
                Remove Project
              </button>
            </div>
        ...
        <div className="section">
          ...
          {formData.education.map((edu, index) => (
            <div key={index} className="education-entry">
              <input
                ...
                value={edu.college}
                onChange={(e) =>
                  handleChange(index, e, "college", "education")
                }
              />
              <input
                type="text"
                ...
                value={edu.course}
                onChange={(e) =>
                  handleChange(index, e, "course", "education")
                }
              />
              <input
                type="text"
          ...
          <button type="button" onClick={() => addArrayField("education")}>
            Add Education
          </button>
        </div>
        ...
        <div className="button-group">
          ...
          >
            {changesReflected ? "Update Resume" : "Generate Again"}
          </button>
        </div>
      </form>
